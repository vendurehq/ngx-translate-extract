...
describe('StringCollection', () => {
	...
	it('should add second source file on duplicate key', () => {
		...
		expect(updatedCollection.get('theKey')).to.deep.equal({
			value: 'theVal',
			sourceFiles: ['path/to/file.ts', 'path/to/another-file.ts'],
		});
	});
	...
	it('should add array of keys with default value', () => {
		...
		expect(collection.values).to.deep.equal({
			key1: { value: '', sourceFiles: ['path/to/some-file.ts'] },
			key2: { value: '', sourceFiles: ['path/to/some-file.ts'] },
		});
	});
	...
	it('should intersect with passed collection', () => {
		...
		expect(collection.intersect(newCollection).values).to.deep.equal({
			red: { value: '', sourceFiles: [''] },
			blue: { value: '', sourceFiles: [''] },
		});
	});

	it('should intersect with passed collection and keep original values', () => {
		collection = new TranslationCollection({
			red: { value: 'rød', sourceFiles: [] },
			green: { value: 'grøn', sourceFiles: [] },
			blue: { value: 'blå', sourceFiles: [] },
		});
		const newCollection = new TranslationCollection({
			red: { value: 'no value', sourceFiles: [] },
			blue: { value: 'also no value', sourceFiles: [] },
		});
		expect(collection.intersect(newCollection).values).to.deep.equal({
			red: { value: 'rød', sourceFiles: [] },
			blue: { value: 'blå', sourceFiles: [] },
